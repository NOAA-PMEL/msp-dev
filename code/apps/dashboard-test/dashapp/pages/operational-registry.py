...
dash.register_page(
    __name__,
    path="/operational-registry",
    title="Operational Devices Registry",  # , prevent_initial_callbacks=True
)
...
def get_layout():
    ...
    layout = html.Div(
        [
            html.H1("Operational Devices Registry"),
            # get_button,
            dbc.Accordion(
                [
                    dbc.AccordionItem(
                        ...
                        title="Operational Devices Definitions",
                    ),
                    dbc.AccordionItem(
                        [
                            dag.AgGrid(
                                id="active-operational-table",
                                rowData=[],
                                columnDefs=[
                                    {
                                        "field": "operational_id",
                                        "headerName": "Operational Device ID",
                                        "filter": True,
                        ...
                        title="Active Operational Devices",
                    ),
                ],
                id="operational-accordion",
            ),
            WebSocket(
                id="ws-operational-registry",
                ...
                url=f"ws://{config.ws_hostname}/msp/dashboardtest/ws/operational-registry/main"
            ),
            ws_send_buffer,
            dcc.Store(id="operational-defs-changes", data=[]),
            dcc.Store(id="active-operational-changes", data=[]),
...
@callback(
    Output("operational-defs-table", "rowData"),
    Input("table-update-interval", "n_intervals"),
    State("operational-defs-table", "rowData")
    # prevent_initial_call=True
)
def update_operational_definitions(count, table_data):
    ...
    try:
        ...
        print(f"operational definition results: {results}")
        if "results" in results and results["results"]:
            for doc in results["results"]:
                if doc is not None:
                    # print(f"doc: {doc}")
                    id = doc["operational_definition_id"]
                    make = doc["make"]
                    model = doc["model"]
                    version = doc["version"]

                    operational_def = {
                        "operational-def-id": id,
                        "make": make,
                        "model": model,
                        "version": version,
                    }
                    if operational_def not in table_data:
                        table_data.append(operational_def)
                        update = True
                    new_data.append(operational_def)
        ...
        print(f"update_operational_definitions error: {e}")
        return dash.no_update


@callback(
    Output("active-operational-table", "rowData"),
    Input("table-update-interval", "n_intervals"),
    State("active-operational-table", "rowData")
)
def update_active_operationals(count, table_data):
    ...
    try:
        ...
        if "results" in results and results["results"]:
            for doc in results["results"]:
                ...
                serial_number = doc["serial_number"]
                version = doc["version"]
                operational_id = "::".join([make, model, serial_number])
                sampling_system_id = "unknown::unknown::unknown"

                operational = {
                    # "sensor_id": f"[{sensor_id}](http://uasdaq.pmel.noaa.gov/uasdaq/dashboard/dash/sensor/{sensor_id})",
                    "operational_id": f"[{operational_id}]({link_url_base}/dash/operational/{operational_id})",
                    # "sensor_id": f"[{sensor_id}]({rel_path}/sensor/{sensor_id})",
                    "make": make,
                    "model": model,
                    "serial_number": serial_number,
                ...
                if operational not in table_data:
                    table_data.append(operational)
                    update = True
                new_data.append(operational)
        ...
        print(f"update_active_operationals error: {e}")
        return dash.no_update
...
@callback(Output("ws-operational-registry", "send"), Input("ws-send-buffer", "children"))
def send(value):
