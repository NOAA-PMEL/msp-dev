...
class ShellyPro2(Controller):
    ...
    def configure(self):
        ...
        try:
            ...
            self.logger.error("ShellyPro2:configure", extra={"error": str(e)})
        ...
        while True:
            for channel in range(0, 2):
                data = {
                    ...
                    "message": json.dumps({"id": channel, "src": f"{self.controller_id_prefix}-status", "method": "Shelly.GetStatus"})
                }
            ...
            try:
                ...
                if self.active() and state_str == "active":
                    if self.polling_task is None or self.polling_task.done():
                        self.logger.info("Starting ShellyPro2 status polling loop.")
                        self.polling_task = asyncio.create_task(self.get_status_loop())
                else:
                    if self.polling_task and not self.polling_task.done():
                        self.logger.info("Stopping ShellyPro2 status polling loop.")
