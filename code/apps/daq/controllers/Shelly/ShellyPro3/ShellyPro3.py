...
class ShellyPro3(Controller):
    ...
    def configure(self):
        ...
        try:
            ...
            # Keep the ShellyPro3's original subscription paradigm
            ...
            self.logger.error("ShellyPro3:configure", extra={"error": str(e)})
            ...
            try:
                ...
                if self.active() and state_str == "active":
                    if self.polling_task is None or self.polling_task.done():
                        self.logger.info("Starting ShellyPro3 status polling loop.")
                        self.polling_task = asyncio.create_task(self.get_status_loop())
                else:
                    if self.polling_task and not self.polling_task.done():
                        self.logger.info("Stopping ShellyPro3 status polling loop.")
    ...
    def default_parse(self, data):
        ...
        try:
            ...
            # ShellyPro3 uses direct channel mapping in the payload typically
            channel = shelly_data.get("id")
            output = shelly_data.get("output")
            
            temp_dict = shelly_data.get("temperature", {})
            ...
            # If we received output state, map it properly (Note: ShellyPro3 JSON mapped 1,2,3)
