...
class SanAce92RF(Operational):
    ...
    def configure(self):
        super(SanAce92RF, self).configure()
    ...
    def check_fan_speed_sp(self, data):
        try:
            ...
            if duty_cycle is not None:
                # Reversible fan logic: 50% DC = 0 SP. Range is -100 to 100.
                sp = (float(duty_cycle) - 50.0) * 2.0
            ...
            for name in self.settings.get_settings().keys():
                if not self.settings.get_health_setting(name):
                    ...
                    if name == "sampling_state":
                        ...
                        try:
                            sp = float(target_val)
                            # Convert +/- 100% SP to 0-100% Duty Cycle
                            pwm_data = (sp / 2.0) + 50.0
                ...
                if self.sampling() and state_str == "sampling":
                    if self.polling_mode == "polled":
                        if self.polling_task is None or self.polling_task.done():
                            self.logger.info("Starting SanAce92RF polling loop.")
                            self.polling_task = asyncio.create_task(self.polling_loop())
                    elif self.polling_mode == "unpolled":
                        if self.polling_task and not self.polling_task.done():
                            self.logger.info("Stopping SanAce92RF polling loop (Unpolled mode active).")
                            self.polling_task.cancel()
                            self.polling_task = None
                else:
                    if self.polling_task and not self.polling_task.done():
                        self.logger.info("Stopping SanAce92RF polling loop.")
                        self.polling_task.cancel()
                        self.polling_task = None
...
async def main(server_config: ServerConfig = None):
    ...
    logger = logging.getLogger(f"SanyoDenki::SanAce92RF::{sn}")

    logger.debug("Starting SanAce92RF")
    inst = SanAce92RF()
    inst.run()
    await asyncio.sleep(2)
    inst.start()
    ...
    while do_run:
        logger.debug("SanAce92RF.run", extra={"do_run": do_run})
        await asyncio.sleep(1)
