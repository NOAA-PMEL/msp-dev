...
class APS3321(Sensor):
    ...
    async def default_data_loop(self):
        record_buffer = None  # Initialize state safely
    ...
    def default_parse(self, data):
        ...
        try:
            ...
            self.var_name = None # Reset every time to prevent TypeErrors
            ...
            if ',C,' in raw_str:
                if ',C,0' in raw_str:
                    ...
                    compiled_record = parts
                    self.C_counter = 0
                    self.array_buffer = [] # HARD RESET on new frame
                else:
                    ...
                    if self.C_counter < 51:
                        self.check_array_buffer(parts, array_cond=False)
                        self.C_counter += 1
                        return None # Changed to return None to avoid partial variable updates
                    else:
                        compiled_record = self.check_array_buffer(parts, array_cond=True)
                        self.array_buffer = []
                        self.C_counter = 0
