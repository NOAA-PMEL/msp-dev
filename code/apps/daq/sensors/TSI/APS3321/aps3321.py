...
class APS3321(Sensor):
    ...
    async def handle_interface_data(self, message: CloudEvent):
        ...
        if message["type"] == det.interface_data_recv():
            try:
                path_id, iface_path = message["path_id"], self.config.interfaces["default"]["path"]
                if path_id == iface_path:
                    await self.default_data_buffer.put(message)
            except KeyError: pass

    async def settings_check(self):
        """Intercepts MQTT settings requests and acknowledge using set_actual."""
        await super().settings_check()
        if not self.settings.get_health():
            for name in self.settings.get_settings().keys():
                if not self.settings.get_health_setting(name):
                    setting_obj = self.settings.get_setting(name)
                    # Safely handle framework dict payloads
            ...
            try:
                ...
                if self.sampling() and state_str == "sampling":
                    ...
                    if self.collecting or not need_start:
                        await self.interface_send_data(data={"data": stop_command})
                        self.logger.info(f"State transitioned to {state_str}. Halting APS sampling.")
                        await asyncio.sleep(2)
            ...
            try:
                ...
                if self.last_record in raw_str:
                    ...
                    if self.sampling():
                        ...
                        record_buffer["variables"]["intN"]["data"], record_buffer["variables"]["dN"]["data"], record_buffer["variables"]["dNdlogDp"]["data"] = intN, dN, dNdlogDp
    ...
    def default_parse(self, data):
        """Optimized parser using variable_types to reduce payload."""
