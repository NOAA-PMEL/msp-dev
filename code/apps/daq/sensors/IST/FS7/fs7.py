...
class FS7(Sensor):
        ...
        "variables": {
            ...
            "sampling_state": {
                ...
                "attributes": {
                    ...
                    "long_name": {"type": "string", "data": "Instrument Operational State"},
                    "allowed_values": {"type": "string", "data": "idle, sampling, maintenance, error"},
                    "default_value": {"type": "string", "data": "sampling"}
                }
            },
            "volts": {
                "type": "float",
    ...
    def __init__(self, config=None, **kwargs):
        super(FS7, self).__init__(config=config, **kwargs)
        self.data_task = None
        self.data_rate = 1
        ...
        self.sensor_definition_file = "IST_FS7_sensor_definition.json"
    ...
    async def handle_interface_data(self, message: CloudEvent):
        await super(FS7, self).handle_interface_data(message)
...
async def main(server_config: ServerConfig = None):
    ...
    logger = logging.getLogger(f"IST::FS7::{sn}")

    logger.debug("Starting FS7")
    inst = FS7()
    # print(inst)
    # await asyncio.sleep(2)
    inst.run()
    ...
    while do_run:
        logger.debug("fs7.run", extra={"do_run": do_run})
        await asyncio.sleep(1)
