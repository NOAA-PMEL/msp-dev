...
class SamplingSystem:
    ...
    async def hydrate_local_variablemaps(self):
        ...
        limits from the Datastore. Uses eventual consistency to wait for 
        the Datastore and Controllers to come online.
        """
                ...
                for valid_time, vm_obj in time_dict.items():
                    ...
                    for vs_name, vs_def in vm_data.get("variablesets", {}).items():
                        for v_name, v_def in vm_data.get("variables", {}).items():
                            ...
                            # Only hydrate direct mappings that are tagged as settings
                            if v_type == "setting" and m_type == "direct":
                                attrs = v_def.get("attributes", {})
                                
                                # Idempotency check: if limits are missing, fetch them
                                if "valid_max" not in attrs:
                                        ...
                                        try:
                                            ...
                                            if resp.status_code == 200 and resp.json().get("results"):
                                                ...
                                                if src_var in hw_vars:
                                                    ...
                                                    # Inject the missing limits into the live VariableMap
                                                    for attr_key in ["valid_min", "valid_max", "step_increment", "units"]:
                                                        if attr_key in hw_attrs and attr_key not in attrs:
                                                            attrs[attr_key] = hw_attrs[attr_key].copy()
                                                            self.logger.info(f"Hydrated {v_name} with {attr_key}: {hw_attrs[attr_key]['data']}")
