...
def create_deployment_card(deployment, platform_info, host_info, child_cards=None, telemetry_cache=None, is_child=False, child_plat_refs=None):
    ...
    # FIX: Hierarchical Health Rollup
        ...
        for proj_ref, deps in projects_grouped.items():
            ...
            def build_cards(d_list, is_child=False):
                ...
                for d in d_list:
                    ...
                    # FIX: Extract child platform refs for the host card lookup
                    child_plat_refs = [c.get("data", {}).get("platform_ref") for c in children]
                    
                    card = create_deployment_card(
                        d, 
                        platform_map.get(plat_ref, {}), 
                        platform_map.get(d.get("data", {}).get("host_platform_ref"), {}), 
                        child_ui, 
                        telemetry_cache, 
                        is_child=is_child,
                        child_plat_refs=child_plat_refs # Pass it here
                    )
