...
def get_all_devices():
    ...
    for d_type in ["sensor", "operational", "controller"]:
        ...
        try:
            response = httpx.get(url, params={"device_type": d_type} if d_type in ["sensor", "operational"] else {}, timeout=30.0)
...
def build_graph_1d(dropdown_list, xaxis="time"):
    return dbc.Card([
        ...
        dcc.Graph(id={"type": "sensor-graph-1d", "index": xaxis}, figure=go.Figure(data=[go.Scatter(x=[], y=[], mode="lines+markers")], layout={"template": "simple_white"}), style={"height": 300}),
    ])
...
def build_graph_2d(dropdown_list, xaxis="time", yaxis="", zaxis=""):
    ...
    return dbc.Card([
        ...
        dbc.Row([
            ...
            dbc.Col(dcc.Graph(id={"type": "graph-2d-heatmap", "index": f"{xaxis}::{yaxis}"}, figure=go.Figure(data=[go.Heatmap(x=[], y=[], z=[], type="heatmap")], layout={"template": "simple_white"}), style={"height": 500})),
            dbc.Col(dcc.Graph(id={"type": "graph-2d-line", "index": f"{xaxis}::{yaxis}"}, figure=go.Figure(data=[go.Scatter(x=[], y=[], mode="lines")], layout={"template": "simple_white"}), style={"height": 500})),
        ])
    ])
...
def render_device_detail(device_uri):
    ...
    if not device_definition:
        L.warning(f"🚨 WARNING: Could not fetch definition for {device_id}")
    ...
    if device_definition:
        ...
        for name, var in device_definition.get("variables", {}).items():
            ...
            if var_type == "setting":
                ...
                # 🟢 EXACT REPLICA OF SENSOR.PY PARSING
                dtype = var.get("type", "unknown")
                data_type = "text"
                ...
                if multi_dim and len(var["shape"]) == 2:
                    ...
                    if dim_2d not in layout_options["layout-2d"]:
                        ...
                        layout_options["layout-2d"][dim_2d] = {"table-column-defs": [{"field": dim_2d, "headerName": dln, "filter": False, "cellDataType": dcd_data_type, "pinned": "left"}], "variable-list": []}
    ...
    return html.Div(key=device_uri, children=[
        ...
        dbc.Accordion(build_tables(layout_options), id="device-data-accordion", className="mb-4", always_open=True),
...
def display_page(pathname):
    ...
    try:
        ...
        L.error(f"Router Exception: {traceback.format_exc()}")
...
def update_device_buffers(event):
    if event and "data" in event:
        try:
            ...
            data_out, settings_out = dash.no_update, dash.no_update
...
def select_graph_1d(y_axis, device_meta, device_definition, graph_id):
    ...
    try:
        ...
        units = ""
        try:
            u_data = device_definition["variables"][y_axis]["attributes"]["units"]["data"]
            if u_data: units = f'({u_data})'
        except Exception: pass
        
        return go.Figure(data=go.Scatter(x=x, y=y, mode="lines+markers"), layout={"xaxis": {"title": "Time"}, "yaxis": {"title": f"{y_axis} {units}".strip()}, "template": "simple_white"})
...
def select_graph_2d(z_axis, device_meta, device_definition, graph_id):
    ...
    scatter = go.Figure(data=[go.Scatter(x=y, y=orig_z[-1] if len(orig_z) > 0 else [])], layout={"template": "simple_white", "xaxis": {"title": f"{y_axis} {y_units}".strip()}, "yaxis": {"title": f"{z_axis} {z_units}".strip()}})
...
def update_table_2d(device_data, row_data_list, col_defs_list, device_definition):
    ...
    new_row_data_list = []
    for col_defs in col_defs_list:
        if not col_defs: new_row_data_list.append(dash.no_update); continue
        ...
        if dim_2d_is_coord: dim_data = device_definition["variables"][dim_2d].get("data", [])
            ...
            if dim_2d not in device_data.get("variables", {}): new_row_data_list.append(dash.no_update); continue
            dim_data = device_data["variables"][dim_2d].get("data")
            if not dim_data: new_row_data_list.append(dash.no_update); continue
...
def update_settings_table(device_settings, row_data_list):
    ...
    for rows in row_data_list:
        if not rows: updated_row_lists.append(dash.no_update); continue
        grid_patched = False
        for row in rows:
