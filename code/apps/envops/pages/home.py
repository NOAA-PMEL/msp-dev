...
def determine_deployment_status(dep_data):
    """Derives deployment status based on current UTC time vs. planned windows."""
    now = datetime.now(timezone.utc)
    
    start_str = dep_data.get("planned_start_time")
    # Favor actual_end_time if the mission ended early, fallback to planned_end_time
    ...
    try:
        # Safely parse ISO 8601 strings (replace Z with +00:00 for strict parser compatibility)
        ...
        L.error(f"Time parsing error for deployment: {e}")
        return "unknown"

def get_status_badge(status):
    status = str(status).lower()
    if status == "active":
        return dbc.Badge("Active", color="success", className="ms-2 shadow-sm")
    elif status == "planned":
        return dbc.Badge("Planned", color="info", className="ms-2 shadow-sm")
    elif status == "completed":
        return dbc.Badge("Completed", color="secondary", className="ms-2 shadow-sm")
...
def create_deployment_card(deployment, platform_info, host_info, child_cards=None):
    """Creates a card for a deployment, optionally rendering nested child deployments."""
    dep_meta = deployment.get("metadata", {})
    dep_data = deployment.get("data", {})
    
    dep_id = dep_meta.get("name", "Unknown ID")
    # status = dep_data.get("status", dep_data.get("deployment_status", "unknown"))
    status = determine_deployment_status(dep_data)
    start_time = dep_data.get("planned_start_time", "TBD")[:10] 
    
    platform_name = platform_info.get("data", {}).get("display_name", dep_data.get("platform_ref", "Unknown Platform"))
    host_name = host_info.get("data", {}).get("display_name", dep_data.get("host_platform_ref", "Unknown Host"))
    ...
    # Append nested children if this platform acts as a host to other deployments
    if child_cards:
...
def update_home_dashboard(n):
    ...
    # 4. Group Deployments by Project
    projects_grouped = {}
    ...
    for proj_ref, deps in projects_grouped.items():
        ...
        # Identify parent/child relationships based on host_platform_ref
        platform_to_dep = {d.get("data", {}).get("platform_ref"): d for d in deps}
        root_deployments = []
        child_deployments = {} # Maps a host_platform_ref to a list of child deployments

        for d in deps:
            host_ref = d.get("data", {}).get("host_platform_ref")
            # If the host_ref is another platform currently deployed in this project, it's a child.
            if host_ref in platform_to_dep:
                ...
                # Otherwise, it's a root deployment (e.g., hosted directly on a vessel or marina)
    ...
    # Wrap projects in a flush accordion so it looks clean
